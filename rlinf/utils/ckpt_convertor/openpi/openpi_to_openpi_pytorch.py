...
"""Convert an OpenPI checkpoint to the OpenPI PyTorch layout.

This implements the internal new-to-old conversion. OpenPI uses
bare ``Pi0`` keys while OpenPI PyTorch uses ``paligemma_with_expert.*`` keys.
OpenPI does not retain OpenPI's separate 1024-wide action-expert token
head, so an OpenPI PyTorch reference model is required to produce a complete
checkpoint.

Within this module, ``new`` denotes the OpenPI layout and ``old``
    ...
    *,
    ...
    for suffix in (".weight", ".bias"):
        ...
        if rlinf_key in openpi_state_dict:
            ...
            ] = openpi_state_dict[rlinf_key]

    if "img.pos_embedding" in openpi_state_dict:
        position_embedding = openpi_state_dict["img.pos_embedding"]
    ...
    for key, tensor in openpi_state_dict.items():
        if key.startswith(
            (
                "action_in_proj",
                "action_out_proj",
...
def _convert_llm_expert(
    ...
    *,
    ...
    for layer_index in range(18):
        ...
        if rlinf_key in openpi_state_dict:
            gating = openpi_state_dict[rlinf_key]
        ...
        if rlinf_key in openpi_state_dict:
            openpi_pytorch_state_dict[f"{openpi_prefix}mlp.down_proj.weight"] = (
                openpi_state_dict[rlinf_key].T.contiguous()
            )
...
def convert_trained_ckpt(
    ...
    """Convert OpenPI trained weights with an OpenPI PyTorch reference."""
    import safetensors.torch

    if str(input_ckpt).endswith(".safetensors"):
        openpi_state_dict = safetensors.torch.load_file(input_ckpt, device="cpu")
    else:
        openpi_state_dict = torch.load(
...
def convert(
    ...
    """Convert OpenPI weights to OpenPI PyTorch without a reference.

    This always raises because an OpenPI PyTorch checkpoint needs the separate
    action-expert head, which OpenPI cannot reconstruct. Use
    ...
    if ACTION_EXPERT_LM_HEAD not in openpi_pytorch_state_dict:
        raise RuntimeError(
            "openpi_to_openpi_pytorch cannot produce a complete OpenPI "
            "PyTorch checkpoint because the action-expert head "
            f"{ACTION_EXPERT_LM_HEAD!r} is not carried by OpenPI. Pass "
...
def add_arguments(parser) -> None:
    """Register the ``openpi_to_openpi_pytorch`` mode arguments."""
    parser.add_argument(
        "--input-model",
        required=True,
        help="OpenPI checkpoint directory, model.safetensors, or model.pt",
    )
    parser.add_argument(
...
def run(args) -> None:
    """Execute ``openpi_to_openpi_pytorch`` from parsed arguments."""
