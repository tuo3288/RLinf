...
from rlinf.data.datasets.openpi.behavior.behavior_sft_dataset import (
    BehaviorSftDataset,
)
...
def build_behavior_sft_dataloader(
    ...
    if data_path is None:
        raise ValueError("openpi BEHAVIOR SFT requires data.train_data_paths.")
    ...
    if use_skill:
        ...
        if len(tasks) != 1:
            raise ValueError(
                "openpi BEHAVIOR SFT use_skill:true supports exactly one task "
        ...
        if not labels:
            raise ValueError(
                "openpi BEHAVIOR SFT use_skill:true requires the reference "
