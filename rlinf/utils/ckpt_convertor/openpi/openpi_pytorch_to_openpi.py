...
"""Convert an OpenPI PyTorch checkpoint to the OpenPI layout.

The OpenPI PyTorch layout uses ``paligemma_with_expert.*`` keys; the
OpenPI layout uses bare ``Pi0`` keys. The
...
denotes the OpenPI layout. The public CLI mode remains
...
def old_to_new_state_dict(
    ...
    for suffix in (".weight", ".bias"):
        ...
        if source_key in openpi_pytorch_state_dict:
            openpi_state_dict["img.stem" + suffix] = openpi_pytorch_state_dict[
                source_key
            ]
    ...
    if source_key in openpi_pytorch_state_dict:
        ...
        openpi_state_dict["img.pos_embedding"] = (
    ...
    for layer_index in range(27):
        ...
        for source_name, target_name in [
            ...
            for suffix in (".weight", ".bias"):
                ...
                if source_key in openpi_pytorch_state_dict:
                    openpi_state_dict[f"{target_prefix}{target_name}{suffix}"] = (
        ...
        if qkv_weights:
            openpi_state_dict[f"{target_prefix}attn.in_proj_weight"] = torch.cat(
                qkv_weights, dim=0
            )
        if qkv_biases:
            openpi_state_dict[f"{target_prefix}attn.in_proj_bias"] = torch.cat(
                qkv_biases, dim=0
            )
        ...
        for suffix in (".weight", ".bias"):
            ...
            if source_key in openpi_pytorch_state_dict:
                openpi_state_dict[f"{target_prefix}attn.out_proj{suffix}"] = (
                    openpi_pytorch_state_dict[source_key]
                )
        ...
        for name in ("fc1", "fc2"):
            for suffix in (".weight", ".bias"):
                ...
                if source_key in openpi_pytorch_state_dict:
                    openpi_state_dict[f"{target_prefix}mlp.{name}{suffix}"] = (
                        openpi_pytorch_state_dict[source_key]
                    )
    ...
    for suffix in (".weight", ".bias"):
        ...
        if source_key in openpi_pytorch_state_dict:
            openpi_state_dict["img.head" + suffix] = openpi_pytorch_state_dict[
                source_key
            ]
    ...
    for layer_index in range(18):
        ...
        for source_name, target_name in [
            ...
            if source_key in openpi_pytorch_state_dict:
                openpi_state_dict[f"{target_prefix}{target_name}.0.scale"] = (
                    openpi_pytorch_state_dict[source_key]
                )

    source_key = _PALI_LLM + "norm.weight"
    if source_key in openpi_pytorch_state_dict:
        openpi_state_dict["llm.final_norms.0.scale"] = openpi_pytorch_state_dict[
            source_key
        ]
    ...
    for layer_index in range(18):
        ...
        if (
            ...
            gate_transposed = openpi_pytorch_state_dict[gate_key].T.contiguous()
            up_transposed = openpi_pytorch_state_dict[up_key].T.contiguous()
            openpi_state_dict[f"{target_prefix}mlps.1.w_gating"] = torch.stack(
                [gate_transposed, up_transposed], dim=0
            )

        down_key = f"{source_prefix}mlp.down_proj.weight"
        if down_key in openpi_pytorch_state_dict:
            openpi_state_dict[f"{target_prefix}mlps.1.w_linear"] = (
                openpi_pytorch_state_dict[down_key].T.contiguous()
            )
        ...
        for source_name, target_name in [
            ...
            source_key = f"{source_prefix}{source_name}.weight"
            if source_key in openpi_pytorch_state_dict:
                openpi_state_dict[f"{target_prefix}{target_name}.1.scale"] = (
                    openpi_pytorch_state_dict[source_key]
                )
    ...
    for suffix in (".weight", ".bias"):
        ...
        if source_key in openpi_pytorch_state_dict:
            openpi_state_dict["llm.final_norms.1.ada_modulation" + suffix] = (
                openpi_pytorch_state_dict[source_key]
            )
    source_key = _GEMMA_EXPERT + "norm.weight"
    if source_key in openpi_pytorch_state_dict:
        openpi_state_dict["llm.final_norms.1.scale"] = openpi_pytorch_state_dict[
            source_key
        ]
    ...
    for key in openpi_pytorch_state_dict:
        if key.startswith(
            ...
            openpi_state_dict[key] = openpi_pytorch_state_dict[key]

    return openpi_state_dict
...
def add_arguments(parser) -> None:
    """Register the ``openpi_pytorch_to_openpi`` mode arguments."""
    parser.add_argument(
        "--input-model",
        required=True,
    ...
    parser.add_argument(
        "--output-model", required=True, help="output OpenPI checkpoint dir"
    )
    parser.add_argument(
...
def run(args) -> None:
    """Execute the ``openpi_pytorch_to_openpi`` mode."""
    convert(
        args.input_model,
