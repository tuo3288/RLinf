...
Five layout convertors (``jax_to_openpi``,
``openpi_pytorch_to_openpi``, ``sft_to_openpi``,
``openpi_to_openpi_pytorch``, ``sft2deploy``) share one core.
``sft_to_openpi`` reads its model architecture from the same OpenPI
