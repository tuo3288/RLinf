...
"""Convert a JAX Pi0/Pi05 checkpoint to the OpenPI layout.
...
def convert_siglip(params: dict) -> dict:
    """Convert SigLIP ViT parameters from JAX to the OpenPI layout."""
...
def convert(
    ...
    *,
    ...
    """Convert a JAX checkpoint dir to an OpenPI checkpoint.
...
def add_arguments(parser) -> None:
    """Register the ``jax_to_openpi`` mode arguments on ``parser``."""
...
def run(args) -> None:
    """Execute the ``jax_to_openpi`` mode from parsed ``args``."""
    convert(
        args.input_model,
