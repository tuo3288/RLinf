...
"""Dual-Franka tcp_rot6d SFT data loader for the openpi path.

The loader owns dataset access, distributed sampling, and collation into the
openpi ``Observation`` type. Input semantics come from the canonical
...
from rlinf.data.datasets.openpi.dual_franka.dual_franka_sft_dataset import (
...
def build_dual_franka_sft_dataloader(
    ...
    """Build the dual-franka openpi loader for the SFT worker."""
    from omegaconf import OmegaConf

    data_path = resolve_lerobot_repo_id(data_paths)
    if data_path is None:
        raise ValueError("openpi dual_franka SFT requires data.train_data_paths.")
